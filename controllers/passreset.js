...
import sendResetMail from "../server/resetpasswrd.js"; // or Resend version below

// Forgot Password Controller
const forgotPassword = async (req, res) => {
  const { email } = req.body;
  let user; // must be defined outside try so it's accessible in catch

  try {
    user = await userModel.findOne({ email });

    // We always respond success (for security reasons)
    if (!user) {
      return res.json({
        success: true,
        message:
...
export { forgotPassword, PasswordResetToken };
